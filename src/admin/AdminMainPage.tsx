        ...
        <div className="flex h-screen overflow-hidden bg-white">
            ...
            >
                ...
                <nav className="p-2 mt-4 space-y-2">
                    ...
                    <div onClick={() => setActive("Dashboard")} className={`${menuItem} ${active === "Dashboard" ? "bg-primary text-white" : "text-black hover:bg-white/10"}`}>
                        <MdDashboard size={22} />
                        {open && <span>Dashboard</span>}
                    </div>

                    <div onClick={() => setActive("Customers")} className={`${menuItem} ${active === "Customers" ? "bg-primary text-white" : "text-black hover:bg-white/10"}`}>
                        <MdPeople size={22} />
                        {open && <span>Customers</span>}
                    </div>

                    <div onClick={() => setActive("Transfer")} className={`${menuItem} ${active === "Transfer" ? "bg-primary text-white" : "text-black hover:bg-white/10"}`}>
                        <MdSwapHoriz size={22} />
                        {open && <span>Transfer</span>}
                    </div>

                    <div onClick={() => setActive("Transactions")} className={`${menuItem} ${active === "Transactions" ? "bg-primary text-white" : "text-black hover:bg-white/10"}`}>
                        <MdReceipt size={22} />
                        {open && <span>Transactions</span>}
                    </div>

                    <div onClick={() => setActive("Settings")} className={`${menuItem} ${active === "Settings" ? "bg-primary text-white" : "text-black hover:bg-white/10"}`}>
                        <MdSettings size={22} />
            ...
            {/* MAIN */}
            <div className="flex-1 flex flex-col bg-white text-black border-l border-black/10">
